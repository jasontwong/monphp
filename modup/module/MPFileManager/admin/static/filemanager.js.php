...
$(function() {
    var files = [],
        root_dir = '<?php echo MPFileManager::get_file_path(); ?>',
        ...
        build_grid = function(data) {
            ...
            for (i in items)
            {
                ...
                {
                    stat.nice_size = '';
                    image_path = '/file/module/MPFileManager/icon-folder.png';
                    tmp_item.removeClass('selectable');
                    if (name == '..')
                    {
                        image_path = '/file/module/MPFileManager/icon-back.png';
                        type = 'parent';
                        stat.nice_mtime = '';
                    }
                }
        ...
        process_action = function() {
            $.post('/admin/rpc/MPFileManager/browser/', { view: view_type, action: action, files: admin.JSON.make(files), dir: dir }, function(data) {
                if (data.success)
                {
                    switch (data.action)
                    {
    ...
    tool_upload_file
        .click(function(){
            ...
            window.open('/admin/mod/MPFileManager/browse_upload/?upload_dir='+dir,'file_upload', win_params.join());
        })
        .text('Upload File')
        .appendTo(tools);
    tool_refresh
    ...
    div_nav
        .append(
            $('<a />')
                .text('<?php echo basename(MPFileManager::get_file_path()); ?>')
                .click(function(){
                    load_dir(root_dir)
                }));
    div_tools
        .append($('<span class="current_path" />').text('<?php echo MPFileManager::get_web_path(); ?>'))
        .append(tools);
    control_delete
        .text('Delete')
        .data('enabled', false)

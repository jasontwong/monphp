...
class Admin
{
    ...
    public function hook_active()
    {
        ...
        {
            Router::add($pattern, $controller, Router::ROUTE_PCRE, Router::PRIORITY_NORMAL, 'admin');
            if (Router::pattern(TRUE) === $pattern)
            {
                // we need correct headers
                $c = Router::controller(TRUE);
                $s = strrpos($c, '.');
                if ($s !== FALSE)
                {
                    $ext = substr($c, $s + 1);
    ...
    public function hook_admin_css()
    {
        return array(
            'screen' => array(
                '/file/module/Admin/js/jquery/themes/base/jquery.ui.core.css',
                '/file/module/Admin/js/jquery/themes/base/jquery.ui.slider.css',
                '/admin/static/Admin/screen.css/',
                '/admin/static/Admin/field.css/'
            )
        );
    }
    ...
    public function hook_admin_js()
    {
        ...
        if (URI_PATH === '/admin/')
        {
            ...
            $js[] = '/admin/static/Admin/dashboard.js/';
        }
    ...
    {
        return array(
            '/file/module/Admin/js/jquery/jquery.js',
            '/file/module/Admin/js/tiny_mce/tiny_mce.js',
            '/file/module/Admin/js/tiny_mce/jquery.tinymce.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.core.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.widget.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.mouse.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.slider.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.tabs.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.sortable.js',
            '/file/module/Admin/js/jquery/ui/jquery.ui.datepicker.js',
            //'/file/module/Admin/js/timepicker.js',
            '/admin/static/Admin/admin.js/',
            '/admin/static/Admin/field.js/'
        );
    }
    ...
    public function hook_data_info()
    {
        ...
        $logo = array(
            ...
            'html_before' => array(
                'data' => '<img src="/file/upload/'.Data::query('Admin', 'logo', 'name').'" /><br />'
            )
        );
        $bgcolor = array(
            'field' => Field::layout(
            ...
            'type' => 'text',
            'value' => array(
                'data' => Data::query('Admin', 'bgcolor')
            )
        );
        $tiny_data = Data::query('Admin', 'tinyMCE');
        if ($tiny_data['theme_advanced_more_colors'] === 'false')
        {
            unset($tiny_data['theme_advanced_more_colors']);
        }
        $tinyMCE = array(
            'field' => Field::layout('tinyMCE'),
            'name' => 'tinyMCE',
            'type' => 'tinyMCE',
            'value' => $tiny_data
        );
    ...
    public function hook_routes()
    {
        ...
        $routes = array(
            ...
            array('#^/admin/settings/[^/]+/$#', $ctrl.'/settings.php', Router::ROUTE_PCRE),
            array('#^/admin/rpc/([^/]+/)+$#', $ctrl.'/rpc.php', Router::ROUTE_PCRE),
            array('#^/admin/module/.+/$#', $ctrl.'/module.php', Router::ROUTE_PCRE),
            array('#^/admin/mod/.+/$#', $ctrl.'/mod.php', Router::ROUTE_PCRE),
        );
        return $routes;
    }
    ...
    public function hook_admin_tinymce()
    {
        ...
        if (is_array(Data::query('Admin', 'tinyMCE')))
        {
            $options = array_merge($options, Data::query('Admin', 'tinyMCE'));
        }
        return $options;
    }
    ...
    private function _rpc_dashboard($data)
    {
        ...
        User::update('setting', 'admin', 'dashboard', $data);
    }
    ...
    {
        $data = (array)json_decode($data['json']);
        User::update('setting', 'admin', 'quicklinks', $data);
        User::update('setting', 'admin', 'quicklinks', $data);
    }
    ...
    private function _rpc_nav($data)
    {
        ...
        User::update('setting', 'admin', 'nav', $data);
        User::update('setting', 'admin', 'nav', $data);
    }
    ...
    public function hook_start()
    {
        if (Router::source() === 'Admin')
        {
            $redirect = (URI_PARTS > 0 && URI_PART_0 === 'admin')
                        ? (URI_PARTS === 1 || URI_PART_1 !== 'login')
                        : FALSE;

            $logged_in = User::perm('admin access') && deka(FALSE, $_SESSION, 'admin', 'logged_in');
    ...
    public function cb_admin_dashboard($modules)
    {
        ...
        $placement = User::setting('admin', 'dashboard');
    ...
    public function cb_admin_login_submit($results)
    {
        ...
        if ($success)
        {
            User::update('setting', 'admin', 'last_login', time());
        }
        return array(
            'login' => $success,
            'messages' => $messages
    ...
    public function cb_admin_nav($menu)
    {
        ...
        if (User::has_perm('admin settings'))
        {
            $nav['Settings'] = array(
        ...
        $settings = array_keys(Module::hook_user('data_info'));
        foreach ($settings as $mod)
        {
            if (!User::perm($mod.' settings') && !User::has_perm('admin settings'))
            {
                continue;
            }

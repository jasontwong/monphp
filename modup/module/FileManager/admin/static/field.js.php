$(function(){
    var add_image = $('<a href="javascript:;">Add</a>'),
        image_container = $('<div class="FileManager_image_container" />'),
        ...
        update_field = function(){
            var el = $(this),
                container = el.closest('.FileManager_image_container'),
                values = [];
            if (el.data('type') == 'caption')
            {
        ...
        save_fields = function(element){
            var el = $(this),
                container = element instanceof jQuery 
                    ? element 
                    : el.closest('.FileManager_image_container'),
        ...
        .click(function(){
            var el = $(this),
                url = el.data('is_single_selection') 
                    ? '/admin/mod/FileManager/browse/single/'
                    : '/admin/mod/FileManager/browse/',
                win_params = [
                    'height=400',
                    'width=850',
                    'scrollbars=yes',
            ...
            $.windowMsg('selected_files', function(data) {
                var browser_data = JSON.parse(data), 
                    files = browser_data.type === 'multi' 
                        ? browser_data.files
                        : [browser_data.files],
                    container = $('input[name="' + browser_data.input_name + '"]').siblings('.FileManager_image_container').data('type', browser_data.type),
    ...
    // }}}
    // {{{ $('input.FileManagerBrowser')
    $('input.FileManagerBrowser')
        .bind('new_clone', function(){
            var el = $(this),
                container = el.prevAll('.FileManager_image_container'),

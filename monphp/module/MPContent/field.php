...
class MPContentField
{
    ...
    public static function field_conclude_fieldtype($data, $args)
    {
        if ($data)
        {
            ...
            if ($meta_caller = MPField::action_array('meta', $type))
            {
     ...
     * 
     * MPContent is used if the content type needs further data extraction, such
     ...
     *
     * The save action is preparation for merging with the MPContentMPFieldMPData
    ...
    public static function field_fieldtype_fieldtype($key, $data)
    {
        ...
        if ($meta_caller = MPField::action_array('meta', $type))
        {
            ...
            {
                $caller = ($post_caller = MPField::action_array('post', $meta['type']))
                    ? $post_caller
                    : MPField::action_array('fallback', 'post');
                if ($caller && ake($mkey, $data))
                {
                    $layout = MPField::layout($meta['type']);
            ...
            if ($type_caller = MPField::action_array('fieldtype', $type))
            {
        ...
        if (eka($_FILES,'data','name',$key))
        {
            ...
            foreach ($names as $k => $name)
            {
                ...
                else
                {
                    ...
                    {
                        $cfdt = Doctrine::getTable('MPContentMPFieldMPData');
                        $field = $cfdt->findCurrentEntryMPFieldMPData($data['_content_entry_meta_id'], $key, 'data')->fetchArray();
